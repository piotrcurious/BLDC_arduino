...
class BLDCMotor:
    def __init__(self, R=1.0, L=0.01, Ke=0.1, Kt=0.1, J=0.01, b=0.01, v_supply=12.0, pole_pairs=4):
    ...
    def update(self, v_phases, dt, load_torque=0.0, friction_coeff=None):
        ...
        for _ in range(num_steps):
            ...
            for i in range(3):
                # Analytical solution for I(t) = V/R + (I0 - V/R)*exp(-R/L*t)
                v_eff = v_phases[i] - vn - e[i]
                target_i = v_eff / self.R
                tau = self.L / self.R
                ...
                joule_heating += self.currents[i]**2 * self.R
...
def main():
    ...
    motor = BLDCMotor()
    dt = 0.001
    pins_out, load_torque, friction_coeff, v_bus_base = {}, 0.0, 0.01, 12.0
    ...
    with open('telemetry.csv', 'w', newline='') as csvfile:
        ...
        while True:
            ...
            if line.startswith("P"):
                ...
                if "OPEN_PHASE_" in line: motor.R = 1e6
                ...
                for i, (hp, lp) in enumerate(phase_map):
                    if pins_out.get(hp, (0,0))[0] == 1: v_phases[i] = pwm * v_bus_actual
                    elif pins_out.get(lp, (0,0))[0] == 1: v_phases[i] = 0.0
                    else:
                        # Diode conduction modeling
                        theta_e = motor.theta * motor.pole_pairs
                        ei = motor.Ke * motor.omega * motor.get_back_emf_factor(theta_e - i * 2*math.pi/3)
                        if motor.currents[i] > 0.01: v_phases[i] = 0.0 # clamp via low-side diode
                        elif motor.currents[i] < -0.01: v_phases[i] = v_bus_actual # clamp via high-side diode
                        else: v_phases[i] = ei
